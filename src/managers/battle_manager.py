...
class BattleManager:
    ...
    def __init__(self, context: Optional[Union['CombatContext', 'IEngine']] = None, log_dir: Optional[str] = None, campaign_engine: Optional['IEngine'] = None) -> None:
        ...
        self.rounds_per_turn = 50 # Default (Legacy)
        self.max_duration_seconds = 60.0 # Default (Live)
        self.time_step = 0.05
        
        if self.context and hasattr(self.context, 'game_config') and self.context.game_config:
            combat_cfg = self.context.game_config.get("combat", {})
            self.rounds_per_turn = combat_cfg.get("rounds_per_turn", 50)
            self.max_duration_seconds = combat_cfg.get("max_duration_seconds", 60.0)
            self.time_step = combat_cfg.get("time_step", 0.05)
    ...
    def resolve_battles_at(self, location: Any, update_indices: bool = True, force_domain: Optional[str] = None, aggressor_faction: Optional[str] = None) -> None:
        ...
        # We rely on _fleets_by_location being up to date.
        # Note: If fleets moved this turn, their location is updated, and indices should be too.
        ...
        if force_domain == "space":
            ...
            # Auto-detect using attributes or class
            # Problem: GraphNode vs Planet vs System
            ...
            # [LOGIC] Domain Isolation
            # If we are in deep space (System), Ground Units (Armies) are cargo. They don't fight alone.
            if is_ground_loc:
                fleets_present = []
            if is_space_loc:
        ...
        with open("debug_bm.txt", "a") as dbg:
             dbg.write("Checking active_battles...\n")
        ...
        with open("debug_bm.txt", "a") as dbg:
             dbg.write(f"Factions Present: {factions_present}\n")
             dbg.write(f"DM is None? {dm is None}\n")
        ...
        for b_id, battle in list(self.active_battles.items()):
            ...
            if use_real_time:
                ...
                winner, survivors, sim_time, stats = resolve_real_time_combat(
                    ...
                    max_time=self.max_duration_seconds,
                    dt=self.time_step
                )
    ...
    def _initialize_new_battle(self, location: Any, fleets: List['Fleet'], armies: List['ArmyGroup'], factions: Set[str], aggressor_faction: Optional[str] = None) -> None:
        ...
        if armies_dict:
            ...
            with open("debug_bm.txt", "a") as dbg:
                 dbg.write(f"Battle added to active_battles. Keys: {list(self.active_battles.keys())}\n")
