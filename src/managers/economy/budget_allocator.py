...
class BudgetAllocator:
    ...
    def process_active_research_spending(self, f_name: str, faction_mgr: 'Faction', budget: int) -> int:
        ...
        spent = 0
        if faction_mgr.requisition > RESEARCH_COST_THRESHOLD and (budget > RESEARCH_COST_THRESHOLD or faction_mgr.requisition > 10000):
            tech_tree = self.engine.tech_manager.faction_tech_trees.get(f_name.lower(), {})
            if tech_tree:
                 all_techs = list(tech_tree.get("techs", {}).keys())
                 
                 # Synthetic Tech Injection
                 synthetic_tiers = ["Tech_Cruiser_Tier_2", "Tech_Capital_Ship_Tier_3"]
                 for st in synthetic_tiers:
                     if st not in all_techs:
                         all_techs.append(st)
                         if st not in tech_tree.get("techs", {}):
                             tech_tree.setdefault("techs", {})[st] = 200

                 # Filter for researchable techs
                 locked_techs = [t for t in all_techs if not faction_mgr.has_tech(t) and self.engine.tech_manager.can_research(f_name, t, faction_obj=faction_mgr)]
                 if locked_techs:
                     locked_techs.sort()
                     # Use economy RNG if available (facade should provide it or we use random)
                     # Assuming BudgetAllocator is initialized with an RNG or accesses it via engine? 
                     # Engine doesn't have economy_rng usually.
                     # We might need to pass rng to execute_budget or init.
                     # For now, using self.recruitment_service.rng as a fallback proxy or random if not clean.
                     # Better: Pass RNG in init.
                     
                     target_tech = locked_techs[0] # Deterministic fallback or need RNG
                     if hasattr(self, 'rng') and self.rng:
                         target_tech = self.rng.choice(locked_techs)
                     
                     base_cost = tech_tree["techs"][target_tech]
                     r_speed = faction_mgr.get_modifier("research_speed_mult", 1.0)
                     tech_cost = int(base_cost * 50 / r_speed)
                     
                     if faction_mgr.can_afford(tech_cost) and (tech_cost <= budget or faction_mgr.requisition > 10000):
                         faction_mgr.deduct_cost(tech_cost)
                         spent += tech_cost
                         self.engine.faction_reporter.log_event(f_name, "research", f"Unlocked technology: {target_tech}")
                         
                         if self.engine.logger:
                             self.engine.logger.economy(f"{f_name} [RP-BUDGET] COMPLETED {target_tech}")

                         faction_mgr.unlock_tech(target_tech, turn=self.engine.turn_counter, tech_manager=self.engine.tech_manager)
                         
                         if self.engine.telemetry:
                             self.engine.telemetry.record_resource_spend(f_name, tech_cost, category="Research")
                             # (Metrics logging omitted for brevity/duplication check)
                             self.engine.telemetry.log_event(EventCategory.ECONOMY, "research_complete", {"tech": target_tech, "cost": tech_cost}, turn=self.engine.turn_counter, faction=f_name)
                             
                         if self.engine.report_organizer:
                              description = f"{f_name} developed {target_tech}"
                              self.engine.report_organizer.log_to_master_timeline(self.engine.turn_counter, "RESEARCH", description)
