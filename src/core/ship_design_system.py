...
class ShipDesigner:
    ...
    def create_design(self, hull: ShipHull, name: str, tech_level: int = 1) -> ShipDesign:
        """
        ...
        for slot in essentials:
            if slot in hull.slots:
                # Find best component
                comp = self._get_best_component(slot, tech_level)
                if comp:
    ...
    def _get_best_component(self, slot_type: str, max_tech: int) -> Optional[ShipComponent]:
        candidates = [c for c in self.registry.values() 
                      if c.slot_type == slot_type and c.tech_level <= max_tech]
